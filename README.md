...
These notebooks demonstrate the application of the extended forecasting strategy using pretrained models. The models are stored in a compressed file named `saved_models.rar`, which can be downloaded from [this Google Drive link](https://drive.google.com/file/d/1GJTFLxFlgh3mFmVrA6kyKQy9nZoyYmFb/view?usp=sharing). To run the notebooks, simply download and extract the contents of `saved_models.rar`, then place them in the same directory as the respective `.ipynb` files. This approach allows you to validate the models' performance on the specified test set time points with ease.
